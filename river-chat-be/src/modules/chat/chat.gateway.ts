...
export class ChatGateway {
  ...
  defaultGroup: string;
  // socket连接钩子
  async handleConnection(@ConnectedSocket() socket: Socket): Promise<string> {
    const userRoom = socket.handshake.query.userId;
    // 连接自动join defaultroom
  ...
  async getAllData(
    ...
    console.log('触发了获取事件', user.password);
    ...
    await this.server
      ...
      .then((sockets) => {
        sockets.forEach((socket) => {
          userIdArr.push(socket.handshake.query.userId);
        });
      })
